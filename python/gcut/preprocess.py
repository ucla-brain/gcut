...
class HandyMan:
        ...
        with open(self.soma_file_path, 'r') as f:
            for l in f:
                ...
                if len(tokens) == 1:
                    ...
                    assert len(tokens) == 3
                    soma_xyzs.append(np.array([float(tokens[0]),
                                               float(tokens[1]),
                                               float(tokens[2])]).reshape(1, 3))
        if len(soma_xyzs) > 0:
    ...
    def extract_soma_ids_from_xyz(self, soma_xyzs):
        ...
        for i in range(soma_xyzs.shape[0]):
            ...
            # row indices of 10 matrix entries closest to soma_xyz
            candidate_indices = np.argsort(d.ravel())[0:10]
            # use the node within candidates with the largest radius
            candidates_ids = tree_matrix[candidate_indices, 0].astype(np.int)
            candidate_radii = tree_matrix[candidate_indices, -2]
            indices_r = np.argsort(candidate_radii)
            for j in range(-1, -11, -1):
                candidate_id = candidates_ids[indices_r[j]]
                if candidate_id not in self.soma_ids:
    ...
    def sink_descendants(self, node_id):
        ...
        while len(descendant_ids) > 0:
            ...
            if descendant_id in self.soma_ids:
                self.soma_ids.remove(descendant_id)
                self.soma_ids.add(node_id)
    ...
    def sink_ancestors(self, node_id):
        ...
        while True:
            ...
            # remove parent
            granparent_id = self.tree_nodes.tree[parent_id].parent_id
            self.tree_nodes.remove_node(parent_id)
            if parent_id in self.soma_ids:
                self.soma_ids.remove(parent_id)
                self.soma_ids.add(node_id)
            parent_id = granparent_id
        return node_id
