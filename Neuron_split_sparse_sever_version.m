function [] = Neuron_split_sparse_sever_version(path_1, path_2, soma_option, poly_option, poly_path, pre_prun_c, post_prun_c, recon_dist)
...
%
% pre_prun_c: This function use the percentage of the distribution of all 
%	neurites' length to prun some neurites which are very short and caused 
%	by noise in image stacks. The input parameter range form 0 to 1 (e.g. 0%~100%).
%	If you do not want to use this function, please input [].
%
...
%------------------------------------------------------------------------
    ...
    for i = 1:1:length(rec_matrix)
        ...
        if ~isempty(post_prun_c)
                ...
                %[neuron_1, neurite_ma_1] = leaf_pruning(neuron_1, neurite_ma_1, leaf_node_1, post_prun_c, 2);
